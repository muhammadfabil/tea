...
const KelolaWaktuBimbingan = () => {
  ...
  const fetchJadwalBimbingan = async (alias) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/waktu_bimbingan/dosen/${alias}`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/waktu_bimbingan/", {
        method: "POST",
        headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/waktu_bimbingan/${selectedJadwal.bimbingan_id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${userData.token}`,
