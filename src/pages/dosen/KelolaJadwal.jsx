...
const KelolaWaktuBimbingan = () => {
  ...
  const fetchJadwalBimbingan = async (alias) => {
    try {
      const response = await fetch(`http://13.236.194.123/waktu_bimbingan/dosen/${alias}`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://13.236.194.123/waktu_bimbingan/", {
        method: "POST",
        headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await fetch(`http://13.236.194.123/waktu_bimbingan/${selectedJadwal.bimbingan_id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${userData.token}`,
  ...
  const handleUpdateAntrianStatus = async (antrianId, statusAction) => {
    ...
    try {
      ...
      const endpoint = `http://13.236.194.123/antrian/f/${antrianId}`;
