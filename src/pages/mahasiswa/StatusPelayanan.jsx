...
const StatusPelayanan = () => {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`wss://13.236.194.123/ws?token=${token}`);
  ...
  const fetchPengajuanData = async () => {
    try {
      ...
      const jenisLayananResponse = await axios.get(`https://13.236.194.123/layanan/jenis`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const response = await axios.get(`https://13.236.194.123/layanan/pengajuan/${nim}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
