...
const AdminPelayanan = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [pengajuanRes, jenisRes] = await Promise.all([
        axios.get("http://127.0.0.1:8000/layanan/pengajuan/all", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get("http://127.0.0.1:8000/layanan/jenis", {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.put(`http://127.0.0.1:8000/layanan/pengajuan/${id}`, {
        status,
        catatan_admin,
        jadwal_pengambilan,
      }, {
